...
async function onPageLoad() {
    ...
    // Load data from local storage if ran before
...
function redirect() {
    window.location.href = `https://api.instagram.com/oauth/authorize?client_id=${clientid}&redirect_uri=https://localhost:5173/&scope=user_profile,user_media&response_type=code`;
}
...
async function getMedia() {
    ...
    if (localStorage.getItem("allMedia")) {
        ...
        // Replace last endpoint's access token with the current one
        if (allMedia.endpoint)
            endpoint = allMedia.endpoint.replace(/(?<=access_token=)[\d\w]*(?=&)/, accessToken);

    } else {

        // Initial endpoint to get 25 most-recent media
        endpoint = `http://localhost:8080/https://graph.instagram.com/me/media?access_token=${accessToken}`;
        allMedia["data"] = {}
    }


    // Get # of posts for progress bar
    if (!(allMedia.mediaCount)) {
        let userResult = await fetch(`https://graph.instagram.com/v17.0/${allMedia.user}?access_token=${accessToken}&fields=media_count`).then((result) => result.json());
        if (userResult.error) {
            handleMediaError(endpoint);
            return;
        }

        allMedia.mediaCount = userResult.media_count;

    }

    let currentPostsLength = 0;
    if (allMedia.data)
        currentPostsLength = getLength();

    // Display progress, disable buttons
    document.getElementById("progress").style.display = "inline-block";
    document.getElementById("login").disabled = true;
    document.getElementById("load").disabled = true;
    document.getElementById("progressBar").style.width = `${(currentPostsLength / allMedia.mediaCount) * 100}%`;
    document.getElementById("progressText").textContent = `${((currentPostsLength / allMedia.mediaCount) * 100).toFixed(1)}% (${currentPostsLength}/${allMedia.mediaCount} posts)`;
    ...
    while (endpoint) {
        ...
        for (let media of result.data) {
            ...
            // Add current post to allMedia's data key: month -> day -> post
            ...
            // Only add media key / update progress bar if not already present in allMedia
            if (!(media.id in allMedia["data"][month][monthDay])) {
                currentPostsLength++;
...
async function loadMediaJSON() {
    ...
    if (allMedia["endpoint"]) {
        ...
        document.getElementById("statusText").style.color = "crimson";
        document.getElementById("statusText").textContent = "This loaded JSON is incomplete and does not have all your posts! To get all your posts, press Get instagram data.";
    }
    updateUI(true);
}
...
function updateUI(fromJSON = false) {
    ...
    // Make calendars visible, update buttons
    ...
    if (!(fromJSON)) {
        ...
        // Change login text to "Continue loading Instagram data" if the loading is incomplete
        if (allMedia["endpoint"]) {
            document.getElementById("login").style.display = "inline"
            document.getElementById("login").textContent = "Continue loading Instagram data"
            document.getElementById("status").style.display = "inline";
            document.getElementById("statusText").style.color = "crimson";
            document.getElementById("statusText").textContent = `Only ${getLength()} posts were retrieved in total. Please refresh and press "Continue loading instagram data" in an hour to continue retrieving all posts.`
        }
        ...
        for (let col = 0; col < 3; col++) {
            ...
            for (let tableRow = 0; tableRow < 5; tableRow++) {
                ...
                for (let tableCol = 0; tableCol < 7; tableCol++) {
                    ...
                    // Add cell data to cell, cell to row
            ...
            // Add table to calendar row
            rowDiv.appendChild(table);

        }

        // Add calendar row to bottom of the calendar side
...
function displayPost(month, key) {
    ...
    for (let id of Object.keys(allMedia["data"][month][key]).sort((a, b) => new Date(allMedia["data"][month][key][a].timestamp).getFullYear() - new Date(allMedia["data"][month][key][b].timestamp).getFullYear())) {
        ...
        // Create post embed for each post on the clicked day
